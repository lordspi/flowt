...
export default function StandaloneTestPage() {
  ...
  const testSeedreamDirect = async () => {
    setLoading(true)
    setResult(null)
  ...
  return (
    <div className="min-h-screen bg-white p-8">
      <div className="max-w-2xl mx-auto">
        <h1 className="text-3xl font-bold mb-4">🔥 COMPLETELY STANDALONE TEST</h1>
        <p className="text-red-600 font-semibold mb-8">
          NO AUTHENTICATION - NO DATABASE - NO PRISMA - PURE API TEST
        </p>
        ...
        <div className="flex gap-4 mb-8">
          ...
          >
            {loading ? 'Generating...' : '🎨 Generate Image (Direct API)'}
          </button>
          
          <button
            onClick={testSeedreamDirect}
            disabled={loading}
            className="bg-green-500 text-white px-6 py-3 rounded-lg hover:bg-green-600 disabled:opacity-50 font-medium"
          >
            {loading ? 'Testing...' : '🧪 Test Seedream Direct'}
          </button>
        </div>
